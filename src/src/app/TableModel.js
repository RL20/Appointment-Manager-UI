...
var urls = ['showFutureAppointmentsList','showEmployeeAppointments','showCustomerAppointments','showAppointmentsHistory', 'showTodayAppointmentsList', 'showAllFutureAbsences', 'showFutureEmployeeAbsence'];
...
var TableModel = React.createClass({
    getInitialState: function() {
    ...
    loadTable: function()
    {
        var isAbsence = this.props.listType == 5 || this.props.listType == 6;



        this.setState({rowType: isAbsence ? "absence" : "appointment"});

        var tableData1 = [];
        // var urls = ['showFutureAppointmentsList','showEmployeeAppointments','showCustomerAppointments','showAppointmentsHistory','showTodayAppointmentsList','showFutureEmployeeAbsence'];
        ...
        if(this.props.listType == 0 ||this.props.listType == 3 ||this.props.listType == 4 ||this.props.listType == 5){
            var _this = this;
        ...
        //     });
            ...
            var personType = this.props.listType == 1 ? "employee" : "customer";
    ...
    getPersonAppointments: function()
    {
        ...
        var isEmployee = this.props.listType == 1 || this.props.listType == 6;

        let payloadPos = isEmployee ? 0 : 1;

        console.log("URL: " + urls[this.props.listType]);
        console.log("payload: " + payload[payloadPos]);
        superagent
            .post(SiteURL.get + urls[this.props.listType])
            .send(payload[payloadPos])
            .set('Accept', 'application/json')
            .end(function(err, res){
                if (err) {
                    console.log('error: ' , err);
        ...
        return new Promise((resolve, reject) => {
            ...
            new Promise((resolve1, reject) => {
                for(var i = 0; i < tableData1.length; i++) {
                    rowState[i] = false;
                    if(this.state.rowType == "appointment")
        ...
        return new Promise((resolve, reject) => {
            ...
            superagent
                ...
                .end(function(err, res){
                    if (err) {
                        ...
                        empNames = JSON.parse(res.text);

                        if(_this.state.rowType == "absence")
                        _this.setNames(JSON.parse(res.text), null, tabledata).then(() => resolve(res.text));
                        else
                        next();
                        // resolve(res.text);
                    }
            ...
            const next = () => {
                superagent
                    ...
                    .send(JSON.stringify(customersIdListPayload))
                    .set({'Accept': 'application/json','Content-Type': 'application/json'})
                    .end(function(err, res){
                        if (err) {
                            console.log('error: ' , err);
                            ...
                            _this.setNames(empNames ,JSON.parse(res.text), tabledata).then(()=>resolve(res.text));
                        }
                    });
    ...
    componentWillMount: function(){
        ...
        //     this.setState({chooseCustomerDailog:true});
        // }
        let personUrls = ["showEmployees","showCustomers"];

        if(this.props.listType == 1 ||this.props.listType == 2 || this.props.listType == 6)
        {
            // alert(this.props.listType - 1);
            let listType= this.props.listType > 2 ? 0 : this.props.listType - 1;
            superagent
                .get(SiteURL.get + personUrls[listType])
                .set('Accept', 'application/json')
                .end(function(err, res){
                    if (err) {
                        console.log('error: ' , err);
    ...
    render: function() {
        ...
        let title = [locale.appointments,locale.byEmployee,locale.byCustomer,locale.history,locale.todaysAppointments,window.strings.absencesPage.listTitle.absences, window.strings.absencesPage.listTitle.byEmployee];
            ...
            <MuiThemeProvider muiTheme={muiTheme}>
                <div>
                    ...
                    <div style={this.props.isDrawerDocked ? window.strings.css.pushContent :null}>
                                    ...
                                    <div   style={this.props.isDrawerDocked ? window.strings.css.drawerOpenedFixedContainer : styles.fixedContainer}>
                                        <Card zDepth={1} id="appTableHead" style={styles.tableHeadContainer}>
                                            ...
                                            <div>
                                                ...
                                                <div>
                                                    <div
                                                        style={{backgroundColor: muiTheme.palette.pickerHeaderColor, color: 'white'}}
                                                        className="table-ro"
                                                    ...
                                                    >
                                                        ...
                                                        > {resultsTitle}
                                                            <FlatButton
                                                                ...
                                                                labelPosition="after"
                                                                icon={<SelectIcon color="white"/>}
                                                                onTouchTap={this.toggleCheckboxes}
                                                            />
                                                        </div>
                                                        <ListItem disabled={true} style={{color: 'inherit'}}
                                                                  leftIcon={this.state.showCheckboxes ? <Checkbox labelStyle={{color: 'white'}} checked={this.state.checkAll} onCheck={this.checkAll} /> : null}>
                                                            <table style={{width: '100%',tableLayout: 'fixed'}}>
                                                                <tr>
                                                                    <td style={{paddingLeft: '8px',textOverflow: 'ellipsis', whiteSpace: 'nowrap', overflow: 'hidden'}}>
                                                                        {this.state.rowType == "appointment" ? window.strings.appointmentsPage.history.time : window.strings.appointmentsPage.history.employee}
                                                                    </td>
                                                                    <td style={{paddingLeft: '17px',textOverflow: 'ellipsis', whiteSpace: 'nowrap', overflow: 'hidden'}}>
                                                                        {this.state.rowType == "appointment" ? window.strings.appointmentsPage.history.employee : window.strings.absecesShowAndEdit.floatingLabelTextFromHour}
                                                                    </td>
                                                                    <td style={{paddingLeft: '28px',textOverflow: 'ellipsis', whiteSpace: 'nowrap', overflow: 'hidden'}}>
                                                                        {this.state.rowType == "appointment" ? window.strings.appointmentsPage.history.customer : window.strings.absecesShowAndEdit.floatingLabelTextToHour}
                                                                    </td>
                                                                </tr>
                                                            </table>
                                                        </ListItem>
                                                ...
                                                >
                                                    ...
                                                    >
                                                        {this.state.tableData.map((row, index) => {
                                                            if (lastDate == null || (row.appointmentDate || row.date) != lastDate || index == 0) {
                                                                lastDate = row.appointmentDate || row.date;
                                                                return (
                                                                    <div>
                                                                        <h2 style={styles.pageTitle}>{row.appointmentDate|| row.date}</h2>
                                                                        <Divider/>
                                                                        <Row hideFilter={this.hideFilter} rowType={this.state.rowType} showAppointment={this.showAppointment} showCheckboxes={this.state.showCheckboxes} obj={row} index={index} key={row.id} checked={this.state.rowState[index]} callback={this.checkRow} />
                                                                        <Divider/></div>
                                                                );
                                                            }
                                                            else {
                        ...
                        { this.state.isItemVisible ?  <EmployeePage component={AppointmentShowAndEdit} ToolsBarComponent={ToolsBar} editUser={this.state.editUser} isNew={this.state.editUser} togglePerson={this.togglePerson} history={this.props.history} personType={this.props.personType} isDrawerDocked={this.props.isDrawerDocked} goBacck={() => this.setState({ showEmployee: false})} obj={this.state.appointmentObj} name={this.state.empName} id={this.state.empId}/>
                            : null}
                    </div>
                    <SearchAndChooseDialog personList={this.state.personList} personType={this.state.personType} handleListClick={(obj) => {this.handlePersonSelected(obj);}} open={this.state.chooseCustomerDailog} close={this.closeChooseCustDialog} />
                </div>
            </MuiThemeProvider>
        );
    }
...
var Row = React.createClass({
    ...
    render: function() {
        ...
        return (
            ...
            <ListItem onTouchTap={this.props.showCheckboxes ? null : (e) => {e.preventDefault(); this.props.showAppointment(this.props.obj)}} className="table-row"
                      leftCheckbox={this.props.showCheckboxes ? <Checkbox checked={this.props.checked} onCheck={this.checkIt} />: null}>
                <table style={{width: '100%',tableLayout: 'fixed'}}>
                    {this.props.rowType == "appointment" ?
                        <tr>
                        ...
                        :
                        <tr>
                            <td style={{paddingLeft: '8px',textOverflow: 'ellipsis', whiteSpace: 'nowrap', overflow: 'hidden'}}>{this.props.obj.employeeId }</td>
                            <td style={{paddingLeft: '17px',textOverflow: 'ellipsis', whiteSpace: 'nowrap', overflow: 'hidden'}}>{this.props.obj.fromHour}</td>
                            <td style={{paddingLeft: '28px',textOverflow: 'ellipsis', whiteSpace: 'nowrap', overflow: 'hidden'}}>{this.props.obj.toHour}</td>
                        </tr>
                    }
